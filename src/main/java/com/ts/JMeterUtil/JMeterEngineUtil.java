...
import static com.ts.javaUtil.FileDirectoryUtil.flushDirectory;
...
public class JMeterEngineUtil {
    ...
    private org.apache.jmeter.protocol.http.control.CookieManager cookieManager;
    ...
    public void setCookieManager() {
        cookieManager = new org.apache.jmeter.protocol.http.control.CookieManager();
    ...
    public static void generateReport(String reportName) {
        try {
            ...
            File reportFile = new File(reportDirectory + reportName + "_JTL_Results" + "/result"
                    + ".csv");
            if (report.exists()) {
                com.ts.javaUtil.FileDirectoryUtil.flushDirectory(report);
            ...
            jMeterEngine.run();
            LOG.info("Performance Execution Done..........");
    ...
    public static void main(String[] args) throws IOException {
        try {
            ...
            setHttpSampler("http", "reqres.in", "/api/users?page=2", "GET");
            setLoopController(2);
            setThreadGroup(10, 5);
            initializeTestPlan("Simple HTTP Test Run using Java API");
            configureTestPlan();
            saveTestPlan("src/test/jmeter/", "new.jmx");
            executeTestWithJMXFile("SimpleHTTPRESTAPIReport", "src/test/jmeter/Jmeter.jmx");
